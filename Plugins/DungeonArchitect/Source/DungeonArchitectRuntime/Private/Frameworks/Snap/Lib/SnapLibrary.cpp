...
void SnapLib::FSnapGraphGenerator::GrowNode(SnapLib::ISnapGraphNodePtr MissionNode, const SnapLib::FGrowthInputState& InputState, SnapLib::FGrowthResult& OutResult) {
    ...
    for (int32 ModuleIdxRef = 0; ModuleIdxRef < PossibleModules.Num(); ModuleIdxRef++) {
        ...
        if (RemoteIncomingDoor.IsValid()) {
            ...
            for (int32 DoorIdxRef = 0; DoorIdxRef < ModuleNode->Doors.Num(); DoorIdxRef++) {
                ...
                if (GetDoorFitConfiguration(RemoteIncomingDoor, IncomingDoor, StaticState.bAllowModuleRotations, NewModuleTransforms)) {
...
TArray<FTransform> SnapLib::FSnapGraphGenerator::GetStartingNodeTransforms(SnapLib::FModuleNodePtr ModuleNode, SnapLib::ISnapGraphNodePtr MissionNode) {
